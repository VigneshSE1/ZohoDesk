...
const UserReward: FC<UserRewardProps> = (props) => {
  ...
  const [privilegeStageName, setPrivilegeStageName] = useState<string>("");

  useEffect(() => {
    getApplicationDetail(props.appId).then((response) => {
      setPrivilegeStageName(response.data.privilegeName);
    });
    getSasToken()
  ...
  return (
    <RewardStyles.RewardContainer>
      <div className="labels">
        <h3>
          {privilegeStageName || "Badge board"} ({countOfRewards || 0})
        </h3>
      </div>
